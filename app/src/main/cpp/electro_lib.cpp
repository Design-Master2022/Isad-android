#include <jni.h>
#include <string>

extern "C"
JNIEXPORT jstring JNICALL
Java_com_design_1master1_isad_model_provider_ElectroLibAccessProvider_getServerBaseUrl(JNIEnv *env,jobject thiz) {

    // TODO Also update base url in Location Update Service and LoginWithOtpViewModel line 136
    std::string s = "https://app.medconfworld.com/";
    return env->NewStringUTF(s.c_str());
}
extern "C"
JNIEXPORT jstring JNICALL
Java_com_design_1master1_isad_model_provider_ElectroLibAccessProvider_getMasterKey(JNIEnv *env,jobject thiz) {
    std::string s = "";
    return env->NewStringUTF(s.c_str());
}
extern "C"
JNIEXPORT jstring JNICALL
Java_com_design_1master1_isad_model_provider_ElectroLibAccessProvider_getSecretKey(JNIEnv *env,jobject thiz) {
    std::string s = "REY0C72CVJBWGY9SLADKTAUDN14T0KZEARRYMH4ARD4QQKJM3RRJINQF1FU9M9RDBWV9XP";
    return env->NewStringUTF(s.c_str());
}